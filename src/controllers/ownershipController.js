...
module.exports.createOwnershipRecords = (req, res, next) => {
    ...
    if (req.body.vehicle_id == undefined || req.body.owner_id == undefined) {
        res.status(400).send({
    ...
    const callback = (error, results, fields) => {
        if (error) {
            ...
            let currentPoints = res.locals.remainingPoints;
            res.status(201).json({
    ...
    const callback = (error, results, fields) => {
        if (error) {
            ...
            if (results[0].vehicle_id_count == 0) {
                ...
                if (results[0].vehicle_id_count == 1) {
