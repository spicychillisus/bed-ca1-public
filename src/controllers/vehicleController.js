...
module.exports.checkForDuplicateVehName = (req, res, next) => {
    const data = {
    ...
    const callback = (error, results, fields) => {
        if (error) {
            ...
            if (results[0].count > 0) {
                res.status(409).json({
                    // this error message pops up if description or vehicle name already exists
                    message: "Vehicle name already exists. Please check your details."
                })
            } else next(); // will continue with the input if description or name does not exist
        }
    }
    model.checkForDuplicateName(data, callback);

}

// checking for duplicate description entries
module.exports.checkForDuplicateVehDescript = (req, res, next) => {
    const data = {
    ...
    const callback = (error, results, fields) => {
        if (error) {
            ...
            if (results[0].count > 0) {
                res.status(409).json({
                    // this error message pops up if description or vehicle name already exists
                    message: "Vehicle description already exists. Please check your details."
                })
            } else next(); // will continue with the input if description or name does not exist
        }
    }
    model.checkForDuplicateDescript(data, callback);
...
module.exports.createNewVehicle = (req, res, next) => {
    ...
    if (req.body.vehicle_name == undefined || req.body.category == undefined || req.body.vehicle_description == undefined || req.body.points_needed == undefined) {
