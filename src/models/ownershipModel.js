...
module.exports.createOwnershipRecords = (data, callback) => {
    const SQLSTATEMENT = `
    INSERT INTO ownership (vehicle_id, owner_id) 
    VALUES (?, ?);
    `;
    const VALUES = [data.vehicle_id, data.owner_id];
...
// for example, if owner_id = 1 already owns vehicle_id = 1.
module.exports.existingRecordCheck = (data, callback) => {
    const SQLSTATEMENT = `
    SELECT EXISTS(SELECT * FROM ownership WHERE vehicle_id = ? AND owner_id = ?)
    AS ownershipVerification;
    `;
    // ownershipVerification is the name of the data where it checks if owner_id = 1 already owns vehicle_id = 1
